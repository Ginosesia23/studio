...
export function CustomSolutions() {
  ...
  return (
    <section id="custom-apps" className="py-28 bg-white relative overflow-hidden">
      <div className="container mx-auto px-6">
        <div className="max-w-4xl mx-auto text-center mb-20">
          <h2 className="text-sm font-bold uppercase tracking-widest text-accent mb-4">Custom Feature Development</h2>
          <h3 className="font-headline text-3xl lg:text-5xl font-bold text-primary mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {solutions.map((item, idx) => (
            <div key={idx} className="p-10 rounded-[2rem] bg-background border border-border hover:border-accent/30 transition-all group">
              <div className="w-14 h-14 bg-primary/5 rounded-2xl flex items-center justify-center mb-8 group-hover:bg-accent/10 transition-colors">
                {item.icon}
              </div>
        ...
        <div className="mt-20 p-8 rounded-3xl bg-accent/5 border border-accent/20 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-accent rounded-xl shadow-md shadow-accent/10">
              <Layers className="text-primary w-6 h-6" />
            </div>
            <div>
              <p className="font-bold text-primary text-lg">Continuous Evolution</p>
              <p className="text-sm text-muted-foreground">Every custom feature comes with managed maintenance.</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <CheckCircle2 className="text-green-500 w-6 h-6" />
            <span className="font-bold text-primary text-base">Reliable long-term support.</span>
          </div>
        </div>
      </div>
    </section>
